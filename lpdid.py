...
def _build_pooled_local(
    ...
    if clean_control == "never_treated":
        ...
        treat_mask = s["D_treat"] == 1
        stable = s[f"CCS_{post_window}"] == 1
        # Matches Stata: D.treat==0 & CCS_j==1 — CCS already ensures stability window
    ...
    good = s.loc[s["D_local"] == 0].groupby(time).size()
    return s.loc[s[time].isin(good.loc[good > 0].index)].reset_index(drop=True)
...
'stabilized'}
    ...
    inference : {'cluster', 'cluster_bootstrap'}
        ``'cluster'``: analytic cluster-robust SE via stacked GMM influence
        functions.  ``'cluster_bootstrap'``: paired cluster bootstrap.
    n_bootstrap : int
    ...
    def __init__(
        ...
        *,
        ...
        # Auto-enable nonabsorbing when clean_control requires it
        if self.clean_control in self._NONABSORBING_CONTROLS and not self.nonabsorbing:
            warnings.warn(
                f"clean_control='{self.clean_control}' requires nonabsorbing=True; "
        ...
        if self.inference not in {"cluster", "cluster_bootstrap"}:
            raise ValueError(
                "inference must be 'cluster' or 'cluster_bootstrap'."
            )
    ...
    def fit(
        ...
        core = self._fit_core(
            data, outcome, unit, time, first_treat, treatment, covariates,
            compute_influence=(self.inference == "cluster"),
        )
        ...
        label = f"LP-DiD ({self.target_estimand.upper()})"
