...
def stacked_influence(
    ...
    """Compute cluster-level influence functions for a stacked GMM estimator."""
    theta_hat = np.asarray(theta_hat, dtype=float)
    ...
    p = len(theta_hat)
    correction = 1.0 if G <= 1 else G / (G - 1.0) * ((n - 1.0) / max(n - p, 1.0))
    sqrt_correction = np.sqrt(correction)

    if target_grad is None:
        psi = sqrt_correction * (S @ Ainv.T) / n
...
def run_cluster_bootstrap(
    ...
    for _ in range(n_bootstrap):
        ...
        sc = boot["scalars"].set_index("term")
